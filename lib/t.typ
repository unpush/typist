...
この後のレッスンでは、練習用ウィンドウの中ほどにテキストを表示し、ウィン
ドウの上の方に指示を表示します。
あなたがすべきことは、画面に表示されたテキストの文字をタイプすることです。
正しい文字を入力した時には特に何もしません。誤った場合にはその部分をハイ
ライト表示し、音を出します。正しくても間違っていても、とにかくタイプを続
けて下さい。Deleteキーは使えません。
...
誤りなくその画面をタイプできたら、次の練習用パターンが表示されます。
...
打ち続けて、できるだけ間違いが少なくなるように頑張って下さい。よく間違え
るのはどのキーでしょうか。

普通のプログラムと同様に、スピード練習の時には、バックスペース、あるいは
Deleteキーを使って、打ち間違えた文字を訂正できます。ただし、エラーのマー
クは消えますが、エラーとしてはカウントされます。
スピード練習の間は、これらのキーを使わないようにすべきだと思います。正し
くタイプできるまで繰り返し練習するべきなのです。

バックスペースや Deleteキーを使う場合は、「;」の指を使って打ち、すぐに
ホームポジションに戻るようにします。\T
この文をタイプしてみましょう:\I
Sarah has liked knitting all her life.\P
少し長い文に挑戦:\I
She looked past her garden to the fields and longed to see the other side.\P
複数の行にまたがる文では、それぞれの行末で改行(Return)を打ちます。\I
...
キーボードにはいくつかの種類がありますが、Typist はそのうち、英語圏で
一般的な 101キーボードと、日本語用の JIS106 キーボードに対応しています。
